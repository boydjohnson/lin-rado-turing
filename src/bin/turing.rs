...
use lin_rado_turing::{machine::Machine, program::{Program, ProgramParseError}, types::{FourState, ThreeState, TwoState, TwoSymbol}};
...
fn main() {
    ...
    match args.next() {
        Some(prog) => {
            match prog.parse::<Program<FourState, TwoSymbol>>() {
                Ok(p) => p,
